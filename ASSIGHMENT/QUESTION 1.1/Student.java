...
{
   //Our fields or information that we will use
   private int id;
   private String name;
   private int age;
   ...
   public void startMenu(String key)
   {
      try
      {
         ...
         else  //This will terminate our program if any number is entereed other than 1,2,3,4,5
         {
            System.exit(0);
         }
      }
   ...
   private void capture()
   {
     ...
     while( !idVerifier)
     {
      try
      {
          ...
          if( idInfo.indexOf(id) > -1 )  //it checks if the id is already in our database
          {
          ...
          if( idVerifier ) {  break;  }  //If it is there, then we are good to go
     ...
     while( !nameVerifier)  //It will give the user multiple chances untill he/she enters the correct name
     {
       name = input.nextLine();
       nameVerifier = verifyName(name);  //This will verify if name is correct according to our specifics. Name should be of at least 2 letters and should only contain letters and . seperator
     ...
     while( age < 16 )
     {
        try
        {
            age = Integer.parseInt(input.nextLine());  // checks if age is a number , if not, it will catch error and allow us to re-enter
    ...
    ageInfo.add(age); // add age to the system ///////////////////////////////////////////////////////
     ...
     System.out.println("The student with student number " + id + " has been successfully captured!");
     promptMenu();
   ...
   private void search()
   {
      ...
      int index = idInfo.indexOf(searchID); //This searchs if the entered Id is inside our array, idinfo
      
      if( index >= 0 ) {  showResults(index);     }
      else{  System.out.println("Student with student id: " + searchID + " was not found!");    }
   ...
   private void delete()
   {
      ...
      {   //These methods remove the whole infortion of a student
          idInfo.remove(idDelete);  nameInfo.remove(idDelete); ageInfo.remove(idDelete); emailInfo.remove(idDelete);  courseInfo.remove(idDelete);      
          
          System.out.println("------------------------------------------------");
          System.out.println("Student with student id: " + idNumber+ " was deleted!");
          System.out.println("------------------------------------------------");
       }
       else{     System.out.println("Student with student id: " + idNumber+ " was NOT deleted!"); }
   ...
   private void Exit()
   {
      ...
      //information hiding, encapsulation, these are our public methods that can be access directly outside our class
      ...
      //End of our required program. Additional methods /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      public boolean verifyName(String n)
      {  
         String nam = n.trim();  // this just cleans the name first by removing spaces
         char[] characters = nam.toCharArray();   // We are spliting name into characters, and then search each of them
         ...
         for( int i = 0; i< characters.length; i++)  // names can cointain . and should be at least bigger than 2 letters
         {
            ...
            if( compare < 46 || ( compare > 46 && compare < 65 ) ||compare > 122 || characters.length < 2  ) {  return false;   }
         }  //The above line is basically stating that names can contain letters and . only
         return true;
      }
      private boolean verifyEmail(String emailAddress)  // We assume that a valid email should have more than 5 character with @
      {
         String[] emailArray = emailAddress.split("");
         
         for(int i = 0; i < emailArray.length; i++)
         {
            if( emailArray[i].equals("@") && emailArray.length > 4){  return true;  } //Email is correct if it has more than 4 letters and @ symbol
         }  return false; // or else it is incorrect
      }
      ...
      private boolean verifyCourse(String course) {    return verifyName(course);   }
}